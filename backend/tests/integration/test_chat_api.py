...
def _make_mock_rag(tokens: list[str] | None = None, sources: list[dict] | None = None):
    ...
    async def _answer(session_id, question, attachments=None):
        for t in tokens:
            yield t
        if sources:
...
def test_sse_content_type(client):
    ...
    with patch("app.api.chat.get_copilot_service", return_value=mock_rag), \
         patch("app.api.chat.get_conversation_manager", return_value=mock_conv):
        resp = client.post(
...
def test_get_messages_returns_list(client):
    ...
    with patch("app.api.chat.get_conversation_manager", return_value=mock_conv):
...
def test_get_messages_hydrates_missing_source_content_from_vector_store(client):
    ...
    with patch("app.api.chat.get_conversation_manager", return_value=mock_conv), \
         patch("app.api.chat.get_copilot_service", return_value=mock_rag):
        resp = client.get("/api/chat/sessions/s1/messages")
...
def test_get_messages_with_attachments(client):
    """GET /messages should return attachments field when present."""
    msgs = [
        Message(
            role="user",
            ...
            additional_kwargs={"attachments": [
                {"id": "abc123", "filename": "report.pdf", "size": 1024}
            ]},
        ),
        Message(role="assistant", content="Summary here", timestamp=datetime.utcnow()),
    ]
    mock_conv = _make_mock_conv(messages=msgs)
    ...
    # First message should have attachments
    assert data[0]["role"] == "user"
    assert data[0]["content"] == "Summarize this document"
    assert data[0]["attachments"] == [
        {"id": "abc123", "filename": "report.pdf", "size": 1024}
    ]

    # Second message should not have attachments
    assert data[1]["role"] == "assistant"
    assert data[1]["content"] == "Summary here"
    assert data[1]["attachments"] is None


def test_get_messages_without_attachments(client):
    """GET /messages should return null attachments when not present."""
    msgs = [
    ...
    assert resp.status_code == 200
    data = resp.json()
    assert data[0]["attachments"] is None
    assert data[1]["attachments"] is None
