...
from app.services.core.conversation import ConversationManager, _messages_to_json, _messages_from_json
...
def mgr():
    ...
    with patch("app.services.core.conversation.SessionLocal") as mock_db_cls:
        mock_db = MagicMock()
...
def test_session_isolation_property(messages_a: list[str], messages_b: list[str]):
    ...
    with patch("app.services.core.conversation.SessionLocal"), \
         patch("app.services.core.conversation.ChatSession"):
        manager = ConversationManager()
        manager._load_from_db = MagicMock(return_value=None)
        manager._persist_session = MagicMock()
        manager._persist_last_active = MagicMock()
