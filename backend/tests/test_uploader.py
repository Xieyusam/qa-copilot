...
from app.services.uploader import DocumentUploader, SUPPORTED_EXTENSIONS
...
# ---------------------------------------------------------------------------
# Unit test: 50MB size limit (Requirement 1.4)
# ---------------------------------------------------------------------------

def test_validate_rejects_file_over_50mb(uploader):
    """Files larger than 50MB must be rejected."""
    over_limit = 51 * 1024 * 1024  # 51 MB
    with pytest.raises(ValueError, match="exceeds"):
        uploader.validate_file("big.pdf", over_limit)


def test_validate_accepts_file_exactly_50mb(uploader):
    """Files exactly at 50MB should be accepted."""
    exactly_50mb = 50 * 1024 * 1024
    uploader.validate_file("big.pdf", exactly_50mb)  # no exception


def test_validate_accepts_file_just_under_50mb(uploader):
    just_under = 50 * 1024 * 1024 - 1
    uploader.validate_file("file.pdf", just_under)  # no exception


def test_validate_rejects_file_one_byte_over_50mb(uploader):
    one_over = 50 * 1024 * 1024 + 1
    with pytest.raises(ValueError):
...
async def test_delete_removes_vector_data():
    ...
    with patch("app.services.uploader.SessionLocal") as mock_db_cls, \
         patch("app.services.uploader.shutil") as mock_shutil, \
         patch("app.services.uploader.Path") as mock_path:
        mock_db = MagicMock()
...
async def test_delete_removes_file_from_disk(tmp_path):
    ...
    with patch("app.services.uploader.settings") as mock_settings, \
         patch("app.services.uploader.SessionLocal") as mock_db_cls:
