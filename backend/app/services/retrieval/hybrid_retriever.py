...
from app.services.retrieval.vector_store import ChunkResult, VectorStore
...
class HybridRetriever:
    ...
    def _tokenize(self, text: str) -> list[str]:
        """
        中文字符 2-gram/3-gram，英文按空格分词。
        兼顾检索质量与无需额外依赖。
        """
        text_lower = text.lower()
        # 提取所有中文连续序列和英文/数字单词
        chinese_seqs = re.findall(r'[\u4e00-\u9fff]+', text_lower)
        ...
        for seq in chinese_seqs:
            for i in range(len(seq)):
                if i < len(seq) - 1:
                    tokens.append(seq[i:i+2])       # 2-gram
                if i < len(seq) - 2:
                    tokens.append(seq[i:i+3])       # 3-gram
    ...
    def add_chunks_to_bm25(self, chunks: list[Chunk], filename: str) -> None:
        ...
        for chunk in chunks:
            ...
            # 中文 2/3-gram 分词，英文空格分词
            tokenized_doc = self._tokenize(chunk.content)
