...
class JiraClient:
    ...
    async def create_issue(
        ...
        client = self._get_client()
        if client is None:
            return "Failed to connect to Jira."
        ...
        try:
            ...
            return f"Error creating Jira issue: {e.text if hasattr(e, 'text') else str(e)}"
        except Exception as e:
            logger.exception("Unexpected error creating Jira issue")
            return f"Error creating Jira issue: {e}"

    async def add_comment(self, issue_key: str, comment: str) -> dict[str, Any] | str:
        ...
        try:
            ...
            return f"Error adding comment: {e.text if hasattr(e, 'text') else str(e)}"
        except Exception as e:
    ...
    async def transition_issue(
        ...
        try:
            client.transition_issue(issue_key, transition=transition_name)
            return f"Issue {issue_key} transitioned to '{transition_name}'"
        except JIRAError as e:
            logger.error(f"Failed to transition issue: {e}")
            return f"Error transitioning issue: {e.text if hasattr(e, 'text') else str(e)}"
        except Exception as e:
            logger.exception("Unexpected error transitioning issue")
