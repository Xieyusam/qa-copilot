...
async def log_node(state: Any) -> Dict:
    """
    Log 节点 - 执行日志分析
    """
    ...
    if not workers:
        if hasattr(state, "get"):
            raise ValueError("log_worker not found in state.workers")
        raise ValueError("log_worker not found")

    log_worker = workers.get("log")
    if not log_worker:
        raise ValueError("log_worker not found in state.workers")
    ...
    logger.info(f"log_node: 执行日志分析 query={query[:50]}...")

    # 执行日志分析
    result = await log_worker.execute(query=query, context={})
